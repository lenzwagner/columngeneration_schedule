...
class MasterProblem:
    def __init__(self, dfData, DemandDF, max_iteration, current_iteration,values):
            ...
            for t in self.days:
                for s in self.shifts:
                    if (i, t, s) in self.start_values:
                        self.model.addLConstr(self.motivation_i[i ,t, s, 1] == self.start_values[i, t, s])
...
for seed in range(259, 301):
    Demand_Dict = generate_cost(14, len(I_list))
    ...
    # Build & Solve MP
    master = MasterProblem(DataDF, Demand_Dict, max_itr, itr, start_values)
    master.buildModel()
