...
def printResults(itr, total_time, time_problem, obj_val_problem, final_obj_cg):
    ...
    print("*{:^88}*".format(""))
    print("*{:^88}*".format("The optimal solution found by normal solver is: " + str(round(obj_val_problem, 3))))
    ...
    gap_str = f"{gap}%"
    if round(final_obj_cg, 1)-round(obj_val_problem, 1) == 0:
    ...
    if round((total_time), 4) < time_problem:
        ...
        print("*{:^88}*".format("Normal solver is faster by " + str(round((total_time), 4) - time_problem) + " seconds"))
    else:
